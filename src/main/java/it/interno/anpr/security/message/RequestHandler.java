...
import it.interno.anpr.security.wss.PDFLoggingInInterceptor;
...
public class RequestHandler {
	...
	protected void setClient(Object wsPort) throws Exception {
		// Client client = ClientProxy.getClient(wsPort);
		// client.getInInterceptors().add(new WSLoggingInInterceptor());
		// client.getInInterceptors().add(new LoggingInInterceptor());
		// client.getInInterceptors().add(new PDFLoggingInInterceptor());
...
//				ConfigHandler.getPathFileKeystore());
		((BindingProvider) wsPort).getRequestContext().put("ws-security.signature.properties",
				ConfigHandler.getProp());

		((BindingProvider) wsPort).getRequestContext().put("ws-security.signature.username",
				ConfigHandler.getParam(ConfigHandler.ALIAS_KEYSTORE_COMUNE));

	}

	protected Object jaxbXMLToObject(InputStream xmlSource, @SuppressWarnings("rawtypes") Class refClass)
		...
		try {
			...
			LOGGER.error("Error converting XML to Object", e);
			throw (e);
		}
	}
}
